...
bool image_contained_in_image(const Mat &M, const Mat &N) {
  ...
  NM.append_matrix(M);
  auto K = NM.graded_kernel();
  auto K_prime = N_copy.graded_kernel();
  auto K_degrees = K.col_degrees;
  auto  K_prime_degrees = K_prime.col_degrees;
  for( auto d: M.col_degrees){
    K_prime_degrees.push_back(d);
  }
  std::sort(K_degrees.begin(), K_degrees.end(), Degree_traits<r2degree>::lex_lambda());
  std::sort(K_prime_degrees.begin(), K_prime_degrees.end(), Degree_traits<r2degree>::lex_lambda());

  if( K_degrees == K_prime_degrees){
    return true;  
  } else {
    return false;
  }
  ...
  /*
  array<int> multi_pivots = array<int>(K.get_num_rows(), vec<int>());
  vec<int> non_zero_columns;
  for(int i = 0; i < K.get_num_cols(); i++){
    int p = K.col_last(i);
    bool found = true;
    while( p != -1 && multi_pivots[p].size() != 0 && found){
      found = false;
      for(int j : multi_pivots[p]){
        if( K.is_admissible_column_operation(j, i) ){
          K.col_op(j, i);
          found = true;
          p = K.col_last(i);
            break;
  ...
  for (int i = threshold; i < NM.get_num_cols(); i++) {
    bool is_in_kernel = false;
    for( int j = 0; j < multi_pivots[i].size(); j++) {
      int col_index = multi_pivots[i][j];
      if( K.col_degrees[col_index] == NM.col_degrees[i] ){
        is_in_kernel = true;
      }
    } 
    if(is_in_kernel == false){
      return false;
    }
  }
  return true;
...
std::pair<Mat, Mat> pruning_pair(Mat &M, const double delta) {
  ...
  for (;;) {
    ...
    std::cout << std::endl;
    // We could use that K_new and K are related
